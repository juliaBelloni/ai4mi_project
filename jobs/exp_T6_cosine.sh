#!/bin/bash
#SBATCH --job-name=exp_T6_cosine_submit
...
SCRIPT="$REPO/jobs/exp_T6_cosine.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_T6_cosine"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_T6_cosine_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        python main.py --dataset SEGTHOR --mode full --epochs 25 \
            ...
            --scheduler cosine --scheduler_t_max 25 \
            --deterministic --seed 43 --augment
        sbatch --job-name=exp_T6_cosine_eval --partition=rome --cpus-per-task=16 \
