#!/bin/bash
#SBATCH --job-name=exp_T4_step_submit
...
SCRIPT="$REPO/jobs/exp_T4_step.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_T4_step"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_T4_step_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        python main.py --dataset SEGTHOR --mode full --epochs 25 \
            ...
            --scheduler step --scheduler_step_size 10 --scheduler_gamma 0.1 \
            --deterministic --seed 43 --augment
        sbatch --job-name=exp_T4_step_eval --partition=rome --cpus-per-task=16 \
