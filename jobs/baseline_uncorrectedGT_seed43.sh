...
SCRIPT="$REPO/jobs/baseline_uncorrectedGT_seed43.sh"
DATA_DIR="$REPO/data/segthor_seed43/baseline"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/baseline"
...
case "$STAGE" in
    prepare)
        ...
        sbatch --job-name=s43_base_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        ...
        sbatch --job-name=s43_base_eval --partition=rome --cpus-per-task=16 \
    ...
    evaluate)
        ...
        python eval.py --pred_folder "$RESULT_DIR/pred_volumes" \
            --gt_pattern "$SOURCE_DIR/train/{id_}/GT.nii.gz" --num_classes 4 \
