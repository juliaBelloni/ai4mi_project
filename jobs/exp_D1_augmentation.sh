#!/bin/bash
#SBATCH --job-name=exp_D1_augmentation_submit
...
SCRIPT="$REPO/jobs/exp_D1_augmentation.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_D1_augmentation"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_D1_augmentation_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        ...
        sbatch --job-name=exp_D1_augmentation_eval --partition=rome --cpus-per-task=16 \
