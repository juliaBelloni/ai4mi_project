#!/bin/bash
#SBATCH --job-name=exp_D2_dropempty90_submit
...
SCRIPT="$REPO/jobs/exp_D2_dropempty90.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_D2_dropempty90"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_D2_dropempty90_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        python main.py --dataset SEGTHOR --mode full --epochs 25 \
            ...
            --scheduler none --deterministic --seed 43 --drop_empty_slices 0.9
        sbatch --job-name=exp_D2_dropempty90_eval --partition=rome --cpus-per-task=16 \
