#!/bin/bash
#SBATCH --job-name=exp_R0_baseline_fixedGT_prep
...
SCRIPT="$REPO/jobs/exp_R0_baseline_fixedGT.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_R0_baseline_fixedGT"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_R0_baseline_fixedGT"
...
case "$STAGE" in
    prepare)
        ...
        if [ ! -d "$DATA_DIR" ]; then
            python slice_segthor.py --source_dir "$SOURCE_DIR" --dest_dir "$DATA_DIR" \
                ...
                --fix_aorta_esophagus --process "$SLURM_CPUS_PER_TASK"
        fi
        sbatch --job-name=exp_R0_baseline_fixedGT_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        ...
        sbatch --job-name=exp_R0_baseline_fixedGT_eval --partition=rome --cpus-per-task=16 \
