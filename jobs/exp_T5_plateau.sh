#!/bin/bash
#SBATCH --job-name=exp_T5_plateau_submit
...
SCRIPT="$REPO/jobs/exp_T5_plateau.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_T5_plateau"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_T5_plateau_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        python main.py --dataset SEGTHOR --mode full --epochs 25 \
            ...
            --scheduler plateau --scheduler_patience 5 --scheduler_gamma 0.1 \
            --deterministic --seed 43 --augment
        sbatch --job-name=exp_T5_plateau_eval --partition=rome --cpus-per-task=16 \
