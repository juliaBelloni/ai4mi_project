...
SCRIPT="$REPO/jobs/exp_D4_augmentation_best_sampling.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
SOURCE_DIR="$REPO/data/segthor_part1"

case "$SAMPLING_METHOD" in
    dropempty)
        NAME="exp_D4_augmentation_dropempty90"
        TRAIN_ARGS=(--augment --drop_empty_slices 0.9)
        ;;
    oversample)
        NAME="exp_D4_augmentation_oversample50"
        TRAIN_ARGS=(--augment --oversample_foreground --oversample_foreground_percent 0.5)
        ;;
    *)
        echo "Set SAMPLING_METHOD to dropempty or oversample." >&2
        exit 2
        ;;
esac

EXPERIMENT_DIR="$REPO/results/segthor_seed43/$NAME"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --export=ALL,SAMPLING_METHOD="$SAMPLING_METHOD" \
            --job-name="${NAME}_train" --partition=gpu_a100 --gpus=1 \
    ...
    train)
        python main.py --dataset SEGTHOR --mode full --epochs 25 \
            ...
            --scheduler none --deterministic --seed 43 "${TRAIN_ARGS[@]}"
        sbatch --export=ALL,SAMPLING_METHOD="$SAMPLING_METHOD" \
            --job-name="${NAME}_eval" --partition=rome --cpus-per-task=16 \
