#!/bin/bash
#SBATCH --job-name=exp_L4_balance_submit
...
SCRIPT="$REPO/jobs/exp_L4_balance.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_L4_balance"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_L4_balance_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        ...
        sbatch --job-name=exp_L4_balance_eval --partition=rome --cpus-per-task=16 \
