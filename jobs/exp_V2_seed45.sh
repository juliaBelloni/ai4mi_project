#!/bin/bash
#SBATCH --job-name=exp_V2_seed45_submit
...
SCRIPT="$REPO/jobs/exp_V2_seed45.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_V2_seed45"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_V2_seed45_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        python main.py --dataset SEGTHOR --mode full --epochs 25 \
            ...
            --scheduler none --deterministic --seed 45 --augment
        sbatch --job-name=exp_V2_seed45_eval --partition=rome --cpus-per-task=16 \
