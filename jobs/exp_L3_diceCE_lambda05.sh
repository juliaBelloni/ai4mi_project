#!/bin/bash
#SBATCH --job-name=exp_L3_diceCE_submit
...
SCRIPT="$REPO/jobs/exp_L3_diceCE_lambda05.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_L3_diceCE_lambda05"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_L3_diceCE_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        python main.py --dataset SEGTHOR --mode full --epochs 25 \
            ...
            --loss_fn dicece --dicece_lambda 0.5 \
            --opt adam --lr 0.0005 --context_slices 2 \
            --scheduler none --deterministic --seed 43 --augment
        sbatch --job-name=exp_L3_diceCE_eval --partition=rome --cpus-per-task=16 \
