#!/bin/bash
#SBATCH --job-name=exp_L6_balance_norm_submit
...
SCRIPT="$REPO/jobs/exp_L6_balance_normalized.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_L6_balance_normalized"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_L6_balance_norm_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        ...
        sbatch --job-name=exp_L6_balance_norm_eval --partition=rome --cpus-per-task=16 \
