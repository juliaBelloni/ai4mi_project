#!/bin/bash
#SBATCH --job-name=exp_T2_lr0p001_submit
...
SCRIPT="$REPO/jobs/exp_T2_lr0p001.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_T2_lr0p001"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_T2_lr0p001_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        python main.py --dataset SEGTHOR --mode full --epochs 25 \
            ...
            --opt adam --lr 0.001 --context_slices 2 \
            --scheduler none --deterministic --seed 43 --augment
        sbatch --job-name=exp_T2_lr0p001_eval --partition=rome --cpus-per-task=16 \
