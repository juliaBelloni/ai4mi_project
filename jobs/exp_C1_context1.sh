#!/bin/bash
#SBATCH --job-name=exp_C1_context1_submit
...
SCRIPT="$REPO/jobs/exp_C1_context1.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_C1_context1"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_C1_context1_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        python main.py --dataset SEGTHOR --mode full --epochs 25 \
            ...
            --loss_fn ce --opt adam --lr 0.0005 --context_slices 1 \
            --scheduler none --deterministic --seed 43 --augment
        sbatch --job-name=exp_C1_context1_eval --partition=rome --cpus-per-task=16 \
