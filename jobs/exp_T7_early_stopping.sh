#!/bin/bash
#SBATCH --job-name=exp_T7_early_stop_submit
...
SCRIPT="$REPO/jobs/exp_T7_early_stopping.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_T7_early_stopping"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_T7_early_stop_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        ...
        sbatch --job-name=exp_T7_early_stop_eval --partition=rome --cpus-per-task=16 \
