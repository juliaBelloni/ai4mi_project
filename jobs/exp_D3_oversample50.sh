#!/bin/bash
#SBATCH --job-name=exp_D3_oversample50_submit
...
SCRIPT="$REPO/jobs/exp_D3_oversample50.sh"
DATA_DIR="$REPO/data/segthor_seed43/exp_P1_HU"
EXPERIMENT_DIR="$REPO/results/segthor_seed43/exp_D3_oversample50"
...
case "$STAGE" in
    submit)
        ...
        mkdir -p "$LOG_DIR"
        sbatch --job-name=exp_D3_oversample50_train --partition=gpu_a100 --gpus=1 \
    ...
    train)
        python main.py --dataset SEGTHOR --mode full --epochs 25 \
            ...
            --oversample_foreground --oversample_foreground_percent 0.5
        sbatch --job-name=exp_D3_oversample50_eval --partition=rome --cpus-per-task=16 \
